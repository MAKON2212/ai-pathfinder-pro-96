...
function CtaBlock({ delay }: { delay: number }) {
  return (
    <motion.div {...fadeUp(delay)} className="mt-6 flex flex-col gap-2">
      <Link
        to="/audit"
...
/** Reusable trust bar */
function TrustBar({ delay }: { delay: number }) {
  return (
    <motion.div {...fadeUp(delay)} className="mt-5 flex items-center gap-3">
      <div className="flex">
        {[
...
function Index() {
  return (
    <div className="mx-auto w-full max-w-md px-3 pb-24 sm:max-w-lg sm:px-5 lg:max-w-5xl lg:px-8">

      {/* ── Hero grid: 2-col on desktop, single col on mobile ── */}
      <div className="mt-8 lg:grid lg:grid-cols-2 lg:gap-16 lg:items-center">
        ...
        <div>
          ...
          <motion.h1
            ...
            style={{ fontSize: "clamp(46px, 8vw, 72px)", color: "#f5ecd7" }}
          >
            What AI{" "}
            <em
          ...
          <motion.p
            {...fadeUp(0.18)}
            className="mt-5 max-w-sm text-[15px] leading-[1.5] tracking-[-0.008em]"
          ...
          {/* CTAs + trust bar — desktop only (left column) */}
        ...
        <motion.div
          {...fadeUp(0.24)}
          className="mt-8 rounded-[20px] p-[22px] lg:mt-0"
          style={{
        ...
        >
          ...
          {/* Big value */}
          <div
            className="relative mt-4 flex items-baseline gap-1.5 font-semibold tabular-nums leading-none tracking-[-0.04em]"
            style={{
              fontSize: 56,
          ...
          >
            ...
            213,480
            <span className="ml-1.5 text-[14px] font-medium" style={{ color: "rgba(245,236,215,0.5)", WebkitTextFillColor: "rgba(245,236,215,0.5)" }}>
              / year
            </span>
          </div>

          {/* Sub-label */}
          <div className="relative mt-2 text-[12px]" style={{ color: "rgba(245,236,215,0.45)" }}>
            Missed AI revenue — based on your industry &amp; team size
          </div>

          {/* Progress bar */}
          <div className="relative mt-5 h-1.5 overflow-hidden rounded-full" style={{ background: "rgba(255,255,255,0.06)" }}>
            <div
          ...
          {/* Foot row */}
          <div className="mt-3 flex justify-between text-[11px]" style={{ color: "rgba(245,236,215,0.45)" }}>
            <span>Range <b style={{ color: "#E8CB85" }}>€148k – €284k</b></span>
            <span>Confidence <b style={{ color: "#E8CB85" }}>94%</b></span>
          </div>

          {/* Divider */}
          <div className="relative mt-5 h-px" style={{ background: "rgba(255,255,255,0.07)" }} />

          {/* Key metrics row */}
          <div className="relative mt-4 grid grid-cols-3 gap-3">
            {[
              { v: "12h", l: "saved / employee / week" },
              { v: "89%", l: "find 3+ quick wins" },
              { v: "€18k", l: "avg. first-year ROI" },
            ].map((m) => (
              <div key={m.v} className="text-center">
                <div
                  className="text-[20px] font-semibold leading-none tracking-[-0.03em]"
                  style={{
                    background: "linear-gradient(180deg, #FFE8A8, #C9A664)",
                    WebkitBackgroundClip: "text",
                    backgroundClip: "text",
                ...
                <div className="mt-1 text-[10px] leading-tight" style={{ color: "rgba(245,236,215,0.45)" }}>{m.l}</div>
              </div>
            ))}
          </div>
        </motion.div>
      ...
      {/* CTAs + trust bar — mobile only (below counter card) */}
      ...
      {/* ── Stats section ── */}
      <motion.div {...fadeUp(0.42)} className="mt-16">
        <div
        ...
        <div className="mt-5 grid grid-cols-2 gap-2.5 lg:grid-cols-3">
          {/* Wide card (spans 2 on mobile, 1 on lg) */}
          <div
            className="col-span-2 rounded-[16px] p-5 lg:col-span-1"
            style={{
          ...
          >
            <div className="mb-3 flex items-end gap-[3px]" style={{ height: 24 }}>
